...
void fe_dndxyz_8_pbr_array(double** dndx, double** dndy, double** dndz, int ele, int nnel, double* dndr, double* dnds, double* dndt, double** invJacobian) {
	for (int i = 0; i < nnel; i++) {
		dndx[ele][i] = (invJacobian[0][0] * dndr[i]) + (invJacobian[0][1] * dnds[i]) + (invJacobian[0][2] * dndt[i]);
		dndy[ele][i] = (invJacobian[1][0] * dndr[i]) + (invJacobian[1][1] * dnds[i]) + (invJacobian[1][2] * dndt[i]);
		dndz[ele][i] = (invJacobian[2][0] * dndr[i]) + (invJacobian[2][1] * dnds[i]) + (invJacobian[2][2] * dndt[i]);
	}
}
