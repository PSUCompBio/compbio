    ...
    if (ndof == 3 && embedded_constraint == true) {
        for (int i = 0; i < num_constraints; ++i) {
            if (cons[i].getName() == "embedded") {
                ...
                fe_getForce_3d_embed(f_tot, u, fext, time_step_counter, host_id, embed_id, correct_vr, include_d, (*embed_map), u_prev, dT, f_damp, d, d_fatigue, d_tot, lambda_min, lambda_max, d_avg, n_load_cycle_full, n_load_cycle_partial, t);
            }
        }
    }
